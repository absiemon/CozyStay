                ...
                <div>
                    ...
                    {subpage === 'profile' && (
                        <div className="text-center max-w-lg mx-auto">
                            <div className={`max-w-md mx-auto mt-10 bg-white rounded-lg overflow-hidden py-3 flex flex-col ${loading && 'items-center'}`}>

                                <div
                                    className={`${!loading? 'relative': ''} w-32 left-40 `}
                                    onMouseEnter={() => setHovering(true)}
                                    onMouseLeave={() => setHovering(false)}
                                >
                                    <img src={user?.avatar} alt="img" className="rounded-full w-32 h-32 " />
                                    {hovering && (
                                        <div className={`${!loading? 'absolute' : ''} inset-0 flex items-center justify-center bg-gray-300 bg-opacity-50 rounded-full `}>
                                            {!loading && <label htmlFor="image" className="cursor-pointer text-gray-600 text-xl p-10 ">
                                                Edit
                                                <input
                                                    type="file"
                                                    id="image"
